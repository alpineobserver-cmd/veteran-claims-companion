...
export async function GET(request:Request,context:Context){
  ...
  let stored;try{stored=await documentStorage().get(document.storageKey)}catch(reason){emitSecurityEvent("document_download_failed",{operation:"GET_OBJECT",scope:"document-download",code:securityEventErrorCode(reason)},"error");return NextResponse.json({error:"Private storage is unavailable."},{status:503})}
