...
export async function DELETE(request:Request,context:Context){
  ...
  try{await deleteObjectAndVerify(documentStorage(),document.storageKey)}catch(reason){await recordStorageReconciliation({userId:session.user.id,operation:"DELETE_OBJECT",scope:"document-delete",entityId:document.id,storageKey:document.storageKey,reason});console.error("Synthetic document object deletion failed",reason instanceof Error?reason.name:"UnknownError");return NextResponse.json({error:"The stored file could not be deleted."},{status:503})}
  ...
  })}catch(reason){await recordStorageReconciliation({userId:session.user.id,operation:"DELETE_DATABASE_RECORD",scope:"document-delete",entityId:document.id,reason});console.error("Synthetic document database deletion failed",reason instanceof Error?reason.name:"UnknownError");return NextResponse.json({error:"The file was removed, but its workspace record still needs cleanup. Try deleting it again."},{status:503})}
