...
export async function DELETE(request:Request){
  ...
  const [documents,legacyUploads,orphanedUploads]=await Promise.all([prisma.document.findMany({where:{userId:session.user.id},select:{storageKey:true}}),prisma.upload.findMany({where:{userId:session.user.id},select:{storageKey:true}}),prisma.storageReconciliationTask.findMany({where:{principalHash,operation:"DELETE_OBJECT",status:"PENDING",storageKey:{not:null}},select:{storageKey:true}})]);
  const objectKeys=[...documents,...legacyUploads,...orphanedUploads].flatMap(item=>item.storageKey?[item.storageKey]:[]);
