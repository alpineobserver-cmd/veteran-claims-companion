...
export async function DELETE(request:Request){
  ...
  if(storageObjects.length){
    try{deletedObjects=await deleteStoredObjectReferencesAndVerify((provider,zone)=>documentStorage(provider,zone),storageObjects)}
    catch(reason){
      const failures=reason instanceof ActiveStorageDeletionError?reason.failedObjects:storageObjects;
      await Promise.all(failures.map(failure=>recordStorageReconciliation({userId:session.user.id,operation:"DELETE_OBJECT",scope:"account-delete",entityId:"account",storageKey:failure.storageKey,storageProvider:failure.storageProvider||undefined,storageZone:failure.storageZone,reason})));
