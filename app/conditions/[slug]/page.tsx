...
export default async function ConditionPage({params}:{params:Promise<{slug:string}>}){
  const {slug}=await params; const c=getCondition(slug); if(!c)notFound(); const criteria=ratingCriteria[slug];
  return <AppShell current="conditions"><div className="condition-detail">
    ...
    <header className="detail-hero"><span className="condition-category">{c.category}</span><h1>{c.name}</h1><p>{c.short}</p><div className="verified"><CalendarDays size={14}/>Criteria checked against eCFR current through July 9, 2026</div></header>
    <div className="detail-layout"><main>
      <DetailSection title="Overview"><p>{c.overview}</p></DetailSection>
      <DetailSection title="Current percentage criteria"><p>{c.schedule}</p>{criteria&&<RatingTable criteria={criteria}/>}<div className="factor-list">{c.ratingFactors.map(x=><div key={x}><span/> {x}</div>)}</div><div className="source-note"><BookOpen size={17}/><div><strong>Read the controlling source</strong><p>Plain-language summaries can omit details and regulations can change.</p><a href={c.sourceUrl} target="_blank" rel="noreferrer">{c.sourceLabel}<ExternalLink size={13}/></a></div></div></DetailSection>
