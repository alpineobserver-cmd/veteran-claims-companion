...
export default async function FormPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <AppShell current="forms"><div className="form-detail">
    <a href="/forms" className="forms-back"><ArrowLeft size={15}/>All forms</a>
    ...
    </main><aside><div className="form-side"><span className="kicker">Related resources</span>{f.related.map(x=><div key={x}>{x}</div>)}</div><div className="form-side form-warning"><Info size={16}/><p>Do not send completed forms or personal information to Veteran Claims Companion. This application does not submit claims.</p></div></aside></div>
