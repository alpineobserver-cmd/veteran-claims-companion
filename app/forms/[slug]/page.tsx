...
export default async function FormPage({params}:{params:Promise<{slug:string}>}){
  ...
  if(!f)notFound();
  const download=getVAFormDownload(slug); const provenance=formProvenance(slug)!;
  return <AppShell current="forms"><div className="form-detail">
    ...
    <header className="form-detail-hero"><div><span className="form-number large">{f.number}</span><span className="form-category">{f.category}</span></div><h1>{f.name}</h1><p>{f.purpose}</p><div className="form-current"><CheckCircle2 size={14}/>{f.status} · {f.revision} · {provenance.contentVersion} · <Link href="/sources">record {provenance.localRecordSha256.slice(0,12)}</Link></div></header>
