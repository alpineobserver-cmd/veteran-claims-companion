...
export default async function IntakePage(){
  ...
  return <AppShell current="intake" user={user}><DocumentIntake initialWorkspaces={workspaces.map(item=>({...item,updatedAt:item.updatedAt.toISOString()}))} initialDocuments={documents.map(item=>({...item,createdAt:item.createdAt.toISOString(),scanCompletedAt:item.scanCompletedAt?.toISOString()||null}))}/><footer className="disclaimer">Test-only document environment. Do not upload real medical records, SSNs, VA file numbers, or another person’s information.</footer></AppShell>;
}
