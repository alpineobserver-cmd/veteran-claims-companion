...
export default async function ClaimBuilderPage({searchParams}:{searchParams:Promise<{claim?:string;new?:string;section?:string}>}){const [session,params]=await Promise.all([auth(),searchParams]);if(!session?.user?.id)redirect("/login?redirectTo=/claim-builder");const user={id:session.user.id,name:session.user.name};const fresh=params.new==="1"&&!params.claim;const initialSection=params.section==="statement"||params.section==="package"?params.section:undefined;return <AppShell current="builder" user={user}><ClaimQuestionnaire user={user} initialClaimId={params.claim} initialSection={initialSection} fresh={fresh}/><footer className="disclaimer">Debrief is an independent educational resource and is not affiliated with the U.S. Department of Veterans Affairs.</footer></AppShell>}
