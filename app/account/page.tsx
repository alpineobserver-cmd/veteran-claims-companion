...
export default async function AccountPage(){
  ...
  const accountData=browserTest?{
    previousWorkspaceCount:0,storedDocumentIds:[] as string[],reworkState:null,approvedPackageIds:[] as string[]
  }:await Promise.all([
    prisma.claim.count({where:{userId:user.id}}),
    prisma.document.findMany({where:{userId:user.id},select:{id:true}}),
    prisma.reworkProfile.findUnique({where:{userId:user.id},select:{state:true}}),
    prisma.reworkPackageSnapshot.findMany({where:{userId:user.id},select:{packageId:true}})
  ]).then(([previousWorkspaceCount,documents,profile,snapshots])=>({
    previousWorkspaceCount,storedDocumentIds:documents.map(item=>item.id),reworkState:profile?.state??null,
    approvedPackageIds:snapshots.map(item=>item.packageId)
  }));
  const summary=summarizeAccountData(accountData);
  return <AppShell current="account" user={{id:user.id,name:user.name,email:user.email,image:user.image}}><div className="account-wrap">
    <header><span className="kicker">Account and data</span><h1>Control your Debrief account</h1><p>Review the claim packages and documents Debrief stores for this account, including approved package history.</p></header>
    ...
    <div className="account-counts" aria-label="Account data summary"><div><Cloud size={18}/><span>Claim packages</span><strong>{summary.claimPackages}</strong></div><div><FileText size={18}/><span>My Documents</span><strong>{summary.documents}</strong></div>{summary.previousWorkspaces>0&&<div><Cloud size={18}/><span>Previous-version workspaces</span><strong>{summary.previousWorkspaces}</strong></div>}</div>
