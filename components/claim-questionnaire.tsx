...
import { AlertTriangle, ArrowLeft, ArrowRight, Check, Clipboard, Cloud, CloudOff, Download, FileDown, History, Info, Link2, LoaderCircle, Plus, RefreshCw, RotateCcw, Save, Sparkles, Trash2 } from "lucide-react";
...
type StatementVersion={id:string;content:string;mode:"ai"|"template"|"edited";createdAt:string};
...
type StoredDraft = { answers: Answers; step: number; furthestStep?:number; statement?:string; statementMode?:StatementMode; timeline?:TimelineEvent[]; evidenceMap?:EvidenceMap; confirmations?:Record<string,boolean>;documentLinks?:Record<string,string[]>;statementVersions?:StatementVersion[];packageStatus?:"planned"|"requested"|"obtained"|"reviewed"|"exported"|"submitted";packageStatusUpdatedAt?:string;packageExportedAt?:string;packageSubmittedAt?:string;buddyStatements?:BuddyStatement[] };
...
export function ClaimQuestionnaire({user,initialClaimId,fresh=false}:{user?:{id:string;name?:string|null};initialClaimId?:string;fresh?:boolean}) {
  ...
  useEffect(()=>{if(!user||!currentClaimId){setDocuments([]);return}let cancelled=false;fetch("/api/documents",{cache:"no-store"}).then(response=>response.json()).then((data:{documents?:DocumentRecord[]})=>{if(!cancelled)setDocuments(data.documents||[])}).catch(()=>{if(!cancelled)setDocuments([])});return()=>{cancelled=true}},[currentClaimId,user]);
...
function Review({answers,condition,timeline,evidenceMap,findings}:{answers:Answers;condition:string;timeline:TimelineEvent[];evidenceMap:EvidenceMap;findings:ReturnType<typeof qualityFindings>}) {
  ...
  return <><span className="question-number">Readiness review</span><h2>See what is ready and what still needs attention</h2><p className="question-help">This review separates missing statement facts from optional improvements and records you are still organizing. It does not predict a claim result.</p>
    ...
    <div className="review-section must-resolve"><h3>Must resolve before drafting</h3>{gaps.length?<ul>{gaps.map(item=><li key={item.field}><div><strong>{item.question}</strong><small>{item.reason}</small></div></li>)}</ul>:<p>Nothing is currently blocking the guided statement draft.</p>}{required.filter(item=>!resolvedFindingIds.has(item.id)).map(item=><article key={item.id}><AlertTriangle size={14}/><div><strong>{item.title}</strong><p>{item.detail}</p></div></article>)}</div>
...
function StatementStep({answers,condition,timeline,update,statement,setStatement,mode,setMode,versions,setVersions}:{answers:Answers;condition:string;timeline:TimelineEvent[];update:Update;statement:string;setStatement:(value:string)=>void;mode:StatementMode;setMode:(mode:StatementMode)=>void;versions:StatementVersion[];setVersions:React.Dispatch<React.SetStateAction<StatementVersion[]>>}){
  ...
  const addVersion=(content:string,versionMode:"ai"|"template"|"edited")=>{if(!content.trim())return;setVersions(current=>{const existing=current[current.length-1];if(existing?.content===content)return current;return[...current,{id:crypto.randomUUID(),content,mode:versionMode,createdAt:new Date().toISOString()}].slice(-20)})};
  ...
  return <><span className="question-number">Personal statement</span><h2>Turn your answers into a cohesive first-person draft</h2><p className="question-help">Debrief checks for the smallest missing facts first. It will not fill gaps or invent details.</p>
    ...
    {statement&&<><div className="statement-editor"><div className="statement-editor-head"><div><span>{modeLabel}</span><small>Review and edit before downloading</small></div><div><button type="button" onClick={()=>addVersion(statement,"edited")}><History size={14}/>Save version</button><button type="button" onClick={copy}><Clipboard size={14}/>{copied?"Copied":"Copy"}</button><button type="button" onClick={download}><Download size={14}/>Download .txt</button></div></div><textarea aria-label="Editable personal statement" value={statement} onChange={event=>{setStatement(event.target.value);setMode("edited")}}/><p><Info size={13}/>Using this draft means you have reviewed it and determined that it accurately reflects your own knowledge and experience.</p></div>{versions.length>0&&<details className="statement-history"><summary><History size={14}/>Revision history · {versions.length} saved {versions.length===1?"version":"versions"}</summary><div>{[...versions].reverse().map((version,index)=><article key={version.id}><div><strong>{index===0?"Most recent saved version":new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"}).format(new Date(version.createdAt))}</strong><small>{version.mode==="ai"?"AI-assisted":version.mode==="template"?"Guided narrative":"Manual revision"} · {version.content.length.toLocaleString()} characters</small></div><button type="button" onClick={()=>restore(version)}><RotateCcw size={13}/>Restore</button><p>{version.content.slice(0,240)}{version.content.length>240?"…":""}</p></article>)}</div></details>}</>}
  </>;
}

function VerifyExport({statement,condition,answers,timeline,evidenceMap,findings,confirmations,setConfirmations,documents,documentLinks,onExport}:{statement:string;condition:string;answers:Answers;timeline:TimelineEvent[];evidenceMap:EvidenceMap;findings:ReturnType<typeof qualityFindings>;confirmations:Record<string,boolean>;setConfirmations:(value:Record<string,boolean>)=>void;documents:DocumentRecord[];documentLinks:Record<string,string[]>;onExport:()=>void}){
