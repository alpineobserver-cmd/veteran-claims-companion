...
type StoredDraft = { answers: Answers; step: number; furthestStep?:number; statement?:string; statementMode?:StatementMode; timeline?:TimelineEvent[]; evidenceMap?:EvidenceMap; confirmations?:Record<string,boolean> };
...
export function ClaimQuestionnaire({user,initialClaimId,fresh=false}:{user?:{id:string;name?:string|null};initialClaimId?:string;fresh?:boolean}) {
  ...
  const draft=useMemo<StoredDraft>(()=>({answers,step,furthestStep,statement,statementMode,timeline,evidenceMap,confirmations}),[answers,step,furthestStep,statement,statementMode,timeline,evidenceMap,confirmations]);
  ...
  function persist(draftStep=step) { const value={answers,step:draftStep,furthestStep:Math.max(furthestStep,draftStep),statement,statementMode,timeline,evidenceMap,confirmations} satisfies StoredDraft;if(!user||!currentClaimId)localStorage.setItem("vcc-claim-draft",JSON.stringify(value));setSaved(true);return value; }
  ...
  function next() { const nextStep=Math.min(steps.length - 1, step + 1); persist(nextStep);setFurthestStep(value=>Math.max(value,nextStep));setStep(nextStep);window.scrollTo({top:0,behavior:"smooth"}); }
  ...
  const canContinue = step === 0 ? Boolean(condition.trim()) : step === 1 ? Boolean(answers.claimType) : step === steps.length-1 ? statementVerified : true;
  ...
  return <div className="builder-wrap">
    ...
    <div className="builder-layout">
      <aside className="step-list" aria-label="Questionnaire progress">{steps.map((label,i)=><button type="button" disabled={i>furthestStep} onClick={()=>goToStep(i)} className={`${i===step?"current":""} ${i<furthestStep?"done":""}`} key={label}><span>{i<furthestStep?<Check size={13}/>:i+1}</span><small>{label}</small></button>)}</aside>
...
function Review({answers,condition,timeline,evidenceMap,findings}:{answers:Answers;condition:string;timeline:TimelineEvent[];evidenceMap:EvidenceMap;findings:ReturnType<typeof qualityFindings>}) {
  ...
  return <><span className="question-number">Readiness review</span><h2>See what is ready and what still needs attention</h2><p className="question-help">This review separates missing statement facts from optional improvements and records you are still organizing. It does not predict a claim result.</p>
    <div className="review-grid"><Summary label="Condition" value={condition}/><Summary label="Claim path" value={answers.claimType}/><Summary label="Support identified" value={`${supported} of ${facts.length}`}/><Summary label="Available records" value={String(available)}/><Summary label="Records pending" value={String(pendingFacts.length)}/><Summary label="Timeline events" value={String(timeline.length)}/></div>
