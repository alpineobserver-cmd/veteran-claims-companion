  ...
  return <div className="rw-shell">
    ...
    <div className="rw-stage" inert={briefingOpen?true:undefined}>
      ...
      <main id="rework-main" tabIndex={-1}>
        ...
        {screen==="dashboard"&&<Dashboard services={services} events={events} documents={documents} sources={sources} leads={leads} claims={claims} packageKind={packageKind} onNavigate={navigate} onOpenClaim={openClaim} onDeleteClaim={deleteClaim}/>}
...
function DeleteClaimDialog({claim,onClose,onDelete}:{claim:ClaimWorkspace;onClose:()=>void;onDelete:()=>void}){
  return <div className="rw-modal-wrap" role="presentation"><section className="rw-modal rw-delete-claim" role="dialog" aria-modal="true" aria-labelledby="delete-claim-title" aria-describedby="delete-claim-copy"><header><div><span className="rw-kicker">Remove from this package</span><h2 id="delete-claim-title">Delete {claim.title}?</h2></div><button type="button" aria-label="Close" onClick={onClose}><X size={18}/></button></header><div className="rw-delete-claim-copy"><span><Trash2 size={18}/></span><p id="delete-claim-copy">This removes the claim draft, statement, and record links from this package. Documents and service history saved to your account will remain available.</p></div><footer><button className="rw-secondary" type="button" autoFocus onClick={onClose}>Keep claim</button><button className="rw-danger" type="button" onClick={onDelete}>Delete claim</button></footer></section></div>;
}
