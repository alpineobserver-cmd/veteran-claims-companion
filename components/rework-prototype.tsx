...
import {
  claimReviewIssueCount,getPackageNextAction,getPackageReadiness,prototypeLeads,prototypeSources,
...
type PackageHistoryItem={id:string;label:string;claimCount:number;approvedAt:string};
...
export function ReworkPrototype({user}:{user:{name:string|null;localTestProfile:boolean}}){
  ...
  const packageReadiness=getPackageReadiness({services,events,documents,claims,sources,packageKind,buddyDecision});
  ...
  useEffect(()=>{
    if(!loaded)return;
    localStorage.setItem(storageKey,JSON.stringify({screen,services,events,documents,sources,leads,claims,packageKind,approved,briefingSeen,buddyDecision,approvalSections,approvalChanges,packageHistory,onboardingComplete,selectedClaimId}));
  },[screen,services,events,documents,sources,leads,claims,packageKind,approved,briefingSeen,buddyDecision,approvalSections,approvalChanges,packageHistory,onboardingComplete,selectedClaimId,loaded]);
  ...
  function reset(){
    ...
    setDocuments([]);setSources(prototypeSources);setLeads([]);setClaims([]);setPackageKind("unsure");setApproved(false);setBriefingSeen(false);setBriefingOpen(true);setBuddyDecision("undecided");setApprovalSections([]);setApprovalChanges([]);setPackageHistory([]);setInspectedSource(null);setFollowUpOpen(false);setOnboardingComplete(false);setSelectedClaimId(null);setNotice("Preview data cleared. Your test profile is empty again.");
  }
  ...
  function startFollowUp(kind:Exclude<ClaimPath,"original"|"unsure">){
    setPackageHistory(current=>[{id:`history-${Date.now()}`,label:packageLabels[packageKind],claimCount:claims.length,approvedAt:"Approved package preview"},...current]);
    setPackageKind(kind);setClaims([]);setLeads([]);setDocuments(current=>current.map(document=>({...document,caseIds:[],claimIds:[]})));setApproved(false);setApprovalSections([]);setApprovalChanges([]);setBuddyDecision("undecided");setSelectedClaimId(null);setFollowUpOpen(false);setScreen("dashboard");setNotice(`Linked ${packageLabels[kind].toLowerCase()} started. Your prior approved package remains in history.`);
  }
  function deleteClaim(claimId:string){
    const claim=claims.find(item=>item.id===claimId);
    if(!claim)return;
    setClaims(current=>current.filter(item=>item.id!==claimId));
    setLeads(current=>current.map(lead=>claimId===`claim-${lead.id}`&&lead.status==="accepted"?{...lead,status:"open"}:lead));
  ...
  return <div className="rw-shell">
    ...
    <div className="rw-stage">
      ...
      <main id="rework-main" tabIndex={-1}>
        ...
        {screen==="intake"&&<Intake services={services} setServices={setServices} events={events} setEvents={setEvents} onboardingComplete={onboardingComplete} onContinue={()=>navigate(onboardingComplete?"dashboard":"documents")} setNotice={setNotice} invalidateApproval={invalidateApproval}/>}
        {screen==="documents"&&<Documents documents={documents} setDocuments={setDocuments} sources={sources} setSources={setSources} onboardingComplete={onboardingComplete} onContinue={()=>onboardingComplete?navigate("dashboard"):finishOnboarding()} setNotice={setNotice} onInspectSource={inspectSource} invalidateApproval={invalidateApproval}/>}
        {screen==="leads"&&<Leads leads={leads} setLeads={setLeads} documents={documents} sources={sources} packageKind={packageKind} claims={claims} setClaims={setClaims} onContinue={()=>navigate("dashboard")} setNotice={setNotice} onInspectSource={inspectSource} invalidateApproval={invalidateApproval}/>}
        {screen==="dashboard"&&<Dashboard services={services} events={events} documents={documents} sources={sources} leads={leads} claims={claims} packageKind={packageKind} packageHistory={packageHistory} onNavigate={navigate} onOpenClaim={openClaim} onDeleteClaim={deleteClaim}/>}
    ...
    {followUpOpen&&<FollowUpPreview onClose={()=>setFollowUpOpen(false)} onStart={startFollowUp}/>}
  </div>;
}
...
function Intake({services,setServices,events,setEvents,onboardingComplete,onContinue,setNotice,invalidateApproval}:{services:ServicePeriod[];setServices:(value:ServicePeriod[])=>void;events:HealthEvent[];setEvents:(value:HealthEvent[])=>void;onboardingComplete:boolean;onContinue:()=>void;setNotice:(value:string)=>void;invalidateApproval:(message:string)=>void}){
...
function Documents({documents,setDocuments,sources,setSources,onboardingComplete,onContinue,setNotice,onInspectSource,invalidateApproval}:{documents:DocumentRecord[];setDocuments:(value:DocumentRecord[]|((current:DocumentRecord[])=>DocumentRecord[]))=>void;sources:SourceReference[];setSources:(value:SourceReference[]|((current:SourceReference[])=>SourceReference[]))=>void;onboardingComplete:boolean;onContinue:()=>void;setNotice:(value:string)=>void;onInspectSource:(sourceId:string)=>void;invalidateApproval:(message:string)=>void}){
  ...
  function retry(id:string){setDocuments(documents.map(item=>item.id===id?{...item,status:"processing"}:item));invalidateApproval("Document analysis was retried.");setNotice("Analysis retry started.");window.setTimeout(()=>{setDocuments(current=>current.map(item=>item.id===id?{...item,status:"ready",insights:[{id:`ins-${id}`,topic:"Lower back",excerpt:"Reports occasional lower-back stiffness.",page:2,confidence:"low",verification:"unreviewed"}]}:item));setSources(current=>[...current.filter(source=>source.documentId!==id),{id:`src-${id}`,kind:"document",label:documents.find(item=>item.id===id)?.name||"Fictional record",documentId:id,location:"Page 2",excerpt:"Reports occasional lower-back stiffness.",confidence:"low",verification:"unreviewed"}])},900)}
  function toggleCaseLink(id:string){setDocuments(documents.map(item=>item.id===id?{...item,caseIds:item.caseIds.includes("case-1")?item.caseIds.filter(caseId=>caseId!=="case-1"):[...item.caseIds,"case-1"]}:item));invalidateApproval("A document package link changed.");setNotice("Document package link updated.")}
  ...
  return <div className="rw-page">
    ...
    <FooterActions next={onContinue} nextLabel="Go to Package Overview" note={onboardingComplete?"Document changes are saved to your account library.":"Documents are optional during setup. You can add them later."}/>
  </div>;
}

function Leads({leads,setLeads,documents,sources,packageKind,claims,setClaims,onContinue,setNotice,onInspectSource,invalidateApproval}:{leads:ClaimLead[];setLeads:(value:ClaimLead[])=>void;documents:DocumentRecord[];sources:SourceReference[];packageKind:ClaimPath;claims:ClaimWorkspace[];setClaims:(value:ClaimWorkspace[])=>void;onContinue:()=>void;setNotice:(value:string)=>void;onInspectSource:(sourceId:string)=>void;invalidateApproval:(message:string)=>void}){
  ...
  return <div className="rw-page">
    ...
    {customOpen&&<AddClaimDialog claims={claims} onClose={()=>setCustomOpen(false)} onAdd={title=>{const id=`claim-custom-${Date.now()}`;setClaims([...claims,{id,title,path:packageKind,progress:0,milestone:"Needs your review",sourceIds:[],documentIds:[],updated:"Just now",draft:createDraft()}]);invalidateApproval("A claim was added to the package.");setCustomOpen(false);setNotice(`${title} added to Your Claims. Review the claim path and details next.`)}}/>}
  </div>;
}

function Dashboard({services,events,documents,sources,leads,claims,packageKind,packageHistory,onNavigate,onOpenClaim,onDeleteClaim}:{services:ServicePeriod[];events:HealthEvent[];documents:DocumentRecord[];sources:SourceReference[];leads:ClaimLead[];claims:ClaimWorkspace[];packageKind:ClaimPath;packageHistory:PackageHistoryItem[];onNavigate:(screen:PrototypeScreen)=>void;onOpenClaim:(claimId:string)=>void;onDeleteClaim:(claimId:string)=>void}){
  ...
  const next=getPackageNextAction({services,events,documents,leads,claims,sources});
  return <div className="rw-page">
    ...
    {packageHistory.length>0&&<section className="rw-package-history" aria-labelledby="package-history-title"><div><span className="rw-kicker">Linked package history</span><h2 id="package-history-title">Prior approved packages remain unchanged</h2><p>The new package can reuse account information without rewriting the approved record.</p></div>{packageHistory.map(item=><article key={item.id}><LockKeyhole size={17}/><span><strong>{item.label}</strong><small>{item.claimCount} {item.claimCount===1?"claim":"claims"}. {item.approvedAt}</small></span><em>Read only</em></article>)}</section>}
...
function PackageReview({services,events,documents,packageKind,claims,sources,approved,setApproved,approvalSections,setApprovalSections,approvalChanges,setApprovalChanges,invalidateApproval,setDownloadOpen,onNavigate,onOpenClaim,onInspectSource,buddyDecision,setBuddyDecision}:{services:ServicePeriod[];events:HealthEvent[];documents:DocumentRecord[];packageKind:ClaimPath;claims:ClaimWorkspace[];sources:SourceReference[];approved:boolean;setApproved:(value:boolean)=>void;approvalSections:ApprovalSection[];setApprovalSections:(value:ApprovalSection[])=>void;approvalChanges:string[];setApprovalChanges:(value:string[])=>void;invalidateApproval:(message:string)=>void;setDownloadOpen:(value:boolean)=>void;onNavigate:(screen:PrototypeScreen)=>void;onOpenClaim:(claimId:string)=>void;onInspectSource:(sourceId:string)=>void;buddyDecision:BuddyDecision;setBuddyDecision:(value:BuddyDecision)=>void}){
  ...
  return <div className="rw-page">
    ...
    {tab==="readiness"?<div className="rw-review-grid">
      <section className="rw-panel"><div className="rw-readiness-head"><div><span className="rw-kicker">Package readiness</span><h2>{unresolved?`${unresolved} remaining`:"Required reviews complete"}</h2></div><span className={unresolved?"attention":"ready"}>{unresolved?"Action required":"Ready for approval"}</span></div>
        ...
        <fieldset className="rw-buddy-choice"><legend>Buddy statement decision <span>Evidence is optional. A decision is required.</span></legend><p>Record whether you plan to add one, cannot obtain one, or do not need one for this package.</p>{[["add","I plan to add one"],["not-available","Not available"],["not-needed","Not for this package"]].map(([value,label])=><label key={value}><input type="radio" name="buddy-decision" value={value} checked={buddyDecision===value} onChange={()=>{setBuddyDecision(value as BuddyDecision);invalidateApproval("Buddy statement decision updated.")}}/><span>{label}</span></label>)}</fieldset>
      </section>
...
function FollowUpPreview({onClose,onStart}:{onClose:()=>void;onStart:(kind:Exclude<ClaimPath,"original"|"unsure">)=>void}){const options:Array<[Exclude<ClaimPath,"original"|"unsure">,string,string]>=[["supplemental","Supplemental claim","Add new and relevant evidence"],["increase","Increased-rating claim","Document that a service-connected condition has worsened"],["contested","Contested decision","Organize the decision and review options"]];return <AccessibleDialog titleId="follow-up-title" className="rw-follow-up" onClose={onClose}><header><div><span className="rw-kicker">Linked follow-up package</span><h2 id="follow-up-title">Start a new package without changing the prior package.</h2></div><button type="button" aria-label="Close follow-up preview" onClick={onClose}><X size={18}/></button></header><p>The approved package remains available as read-only history. Account service and health information stays available.</p><div className="rw-follow-up-options">{options.map(([kind,title,copy])=><button type="button" key={kind} onClick={()=>onStart(kind)}><strong>{title}</strong><small>{copy}</small><ArrowRight size={15}/></button>)}</div><footer><button className="rw-secondary" type="button" onClick={onClose}>Return to package</button></footer></AccessibleDialog>}
