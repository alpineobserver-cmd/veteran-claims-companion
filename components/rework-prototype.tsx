  ...
  return <div className="rw-shell">
    ...
    <aside className={menuOpen?"open":""} aria-label="Claim package navigation">
      ...
      <div className="rw-utility-nav">
        ...
        <Link href="/exposure-record-check" target="_blank" rel="noreferrer"><Radar size={16}/><strong>Exposure Checker</strong><span className="sr-only">, opens in a new tab</span></Link>
        <Link href="/conditions" target="_blank" rel="noreferrer"><BookOpenCheck size={16}/><strong>Conditions Library</strong><span className="sr-only">, opens in a new tab</span></Link>
        <Link href="/forms" target="_blank" rel="noreferrer"><Files size={16}/><strong>Forms Guide</strong><span className="sr-only">, opens in a new tab</span></Link>
        <Link href="/support" target="_blank" rel="noreferrer"><LifeBuoy size={16}/><strong>Help &amp; Guide</strong><span className="sr-only">, opens in a new tab</span></Link>
        <Link href="/support#content-correction" target="_blank" rel="noreferrer"><MessageSquareText size={16}/><strong>Send Feedback</strong><span className="sr-only">, opens in a new tab</span></Link>
      </div>
...
function Dashboard({services,events,documents,sources,leads,claims,packageKind,activePackageId,packages,readOnly,onSwitchPackage,onNavigate,onOpenClaim,onDeleteClaim}:{services:ServicePeriod[];events:HealthEvent[];documents:DocumentRecord[];sources:SourceReference[];leads:ClaimLead[];claims:ClaimWorkspace[];packageKind:ClaimPath;activePackageId:string;packages:PackageRecord[];readOnly:boolean;onSwitchPackage:(packageId:string)=>void;onNavigate:(screen:PrototypeScreen)=>void;onOpenClaim:(claimId:string)=>void;onDeleteClaim:(claimId:string)=>void}){
  ...
  return <div className="rw-page">
    ...
    <section className="rw-tools"><div><span className="rw-kicker">Connected tools</span><h2>Your account details travel with you</h2><p>Reference tools open in a new tab, so this package and your current place remain open.</p></div><nav aria-label="Debrief guides"><Link href="/exposure-record-check" target="_blank" rel="noreferrer"><Radar size={18}/><span><strong>Exposure Checker</strong><small>Starts with {services.length} saved service {services.length===1?"period":"periods"}.</small></span><ExternalLink size={14}/><span className="sr-only">Opens in a new tab</span></Link><Link href="/conditions" target="_blank" rel="noreferrer"><BookOpenCheck size={18}/><span><strong>Conditions library</strong><small>Explore conditions without adding them to your package.</small></span><ExternalLink size={14}/><span className="sr-only">Opens in a new tab</span></Link><Link href="/forms" target="_blank" rel="noreferrer"><Files size={18}/><span><strong>Forms guide</strong><small>Shows forms relevant to this package type.</small></span><ExternalLink size={14}/><span className="sr-only">Opens in a new tab</span></Link></nav></section>
