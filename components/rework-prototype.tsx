...
import {
  prototypeLeads,prototypeSources,
...
export function ReworkPrototype({user}:{user:{name:string|null;localTestProfile:boolean}}){
  const [screen,setScreen]=useState<PrototypeScreen>("intake");
  ...
  function finishOnboarding(){
    ...
    setLeads(hasKneeInformation?[{...prototypeLeads[0],summary:"A knee event appears in the health information you added during setup.",sourceIds:["src-knee-intake"],missing:["Current symptoms","Supporting records"],status:"open"}]:[]);setOnboardingComplete(true);navigate("dashboard");setNotice("Your foundation is ready, and your first claim package has been started.");
  }
  ...
  return <div className="rw-shell">
    ...
    <div className="rw-stage" inert={briefingOpen?true:undefined}>
      <header className="rw-topbar">
        ...
        <div><span>{onboardingComplete?"Active package":"New package setup"}</span><strong>{onboardingComplete?"Initial disability claim":"Build your foundation"}</strong></div>
...
function MissionBriefing({onClose,firstRun}:{onClose:(doNotShowAgain:boolean,start:boolean)=>void;firstRun:boolean}){
  ...
  return <div className="rw-modal-wrap rw-briefing-wrap" role="presentation">
    <section ref={dialogRef} className="rw-modal rw-briefing" role="dialog" aria-modal="true" aria-labelledby="mission-briefing-title" aria-describedby="mission-briefing-intro">
      <header><div><span className="rw-kicker">Mission briefing</span><h2 id="mission-briefing-title" ref={titleRef} tabIndex={-1}>A well-supported original claim connects three things.</h2></div><button type="button" aria-label="Close mission briefing" onClick={()=>onClose(true,false)}><X size={19}/></button></header>
      ...
      <div className="rw-briefing-end">
        <p>{firstRun?"Next, add the service and health details you remember. You can return to this explanation from Help at any time.":"This explanation remains available here without occupying a permanent navigation item."}</p>
        <footer><button className="rw-primary" type="button" onClick={()=>onClose(true,firstRun)}>{firstRun?"Start intake":"Return to package"} <ArrowRight size={15}/></button></footer>
      </div>
    </section>
  </div>;
}
...
function Intake({services,setServices,events,setEvents,onContinue,setNotice}:{services:ServicePeriod[];setServices:(value:ServicePeriod[])=>void;events:HealthEvent[];setEvents:(value:HealthEvent[])=>void;onContinue:()=>void;setNotice:(value:string)=>void}){
  ...
  return <div className="rw-page">
    ...
    <div className="rw-intake-grid">
      <section className="rw-panel"><PanelHead icon={History} title="Service history" count={`${services.length} ${services.length===1?"period":"periods"}`} action="Add service period" onAction={()=>setServiceEditor("new")}/>{services.length?services.map(item=><TimelineRow key={item.id} title={item.location} meta={`${item.kind}, ${formatRange(item.start,item.end)}`} copy={`${item.role}. ${item.exposures}`} approximate={item.approximate} onEdit={()=>setServiceEditor(item.id)}/>):<IntakeEmpty icon={History} title="No service history yet" copy="Add a duty station, deployment, or TDY. Start with whichever period is easiest to remember." action="Add service period" onAction={()=>setServiceEditor("new")}/>}</section>
      <section className="rw-panel"><PanelHead icon={BookOpenCheck} title="Health timeline" count={`${events.length} ${events.length===1?"event":"events"}`} action="Add health event" onAction={()=>setEventEditor("new")}/>{events.length?events.map(item=><TimelineRow key={item.id} title={item.title} meta={`${item.type}, ${friendlyDate(item.date)}`} copy={item.details} approximate={item.approximate} onEdit={()=>setEventEditor(item.id)}/>):<IntakeEmpty icon={BookOpenCheck} title="No health events yet" copy="Add an injury, illness, surgery, or treatment. Approximate dates are acceptable." action="Add health event" onAction={()=>setEventEditor("new")}/>}</section>
    </div>
...
function Documents({documents,setDocuments,onContinue,setNotice,onInspectSource}:{documents:DocumentRecord[];setDocuments:(value:DocumentRecord[])=>void;onContinue:()=>void;setNotice:(value:string)=>void;onInspectSource:(sourceId:string)=>void}){
  ...
  return <div className="rw-page">
    ...
    <FooterActions next={onContinue} nextLabel="Finish setup" note="Documents are optional during setup. You can add them later."/>
  </div>;
}

function Leads({leads,setLeads,documents,claims,setClaims,setChecks,onContinue,setNotice,onInspectSource}:{leads:ClaimLead[];setLeads:(value:ClaimLead[])=>void;documents:DocumentRecord[];claims:ClaimWorkspace[];setClaims:(value:ClaimWorkspace[])=>void;setChecks:(value:PackageReadinessItem[])=>void;onContinue:()=>void;setNotice:(value:string)=>void;onInspectSource:(sourceId:string)=>void}){
...
function Dashboard({services,documents,leads,claims,onNavigate,onOpenClaim}:{services:ServicePeriod[];documents:DocumentRecord[];leads:ClaimLead[];claims:ClaimWorkspace[];onNavigate:(screen:PrototypeScreen)=>void;onOpenClaim:(claimId:string)=>void}){
  ...
  return <div className="rw-page">
    <PageHead kicker="Initial disability claim" title="Your claim package, organized into action." copy="Manage the individual claims, reusable records, and remaining work for this VA submission."/>
    ...
    <div className="rw-metrics"><div><span>Account foundation</span><strong>{services.length} service periods</strong><small>Reusable across future packages</small></div><div><span>Document analysis</span><strong>{documents.filter(item=>item.status==="ready").length} of {documents.length} ready</strong><small>{documents.some(item=>item.status==="failed")?"One needs attention":"All processing complete"}</small></div><div><span>Claim leads</span><strong>{accepted} accepted</strong><small>{leads.filter(item=>item.status==="open").length} still to review</small></div><div><span>Package status</span><strong>{claims.length?"Needs review":"No claims added"}</strong><small>{claims.length?"Approval comes after readiness":"Add a claim when you are ready"}</small></div></div>
  ...
  return <div className="rw-page">
    ...
    <div className="rw-workspace-layout">
      <section className="rw-workspace-form">
        ...
        <WorkspaceField number="1" title="Service event" source="Enter or confirm from your health timeline" value={draft.serviceEvent} placeholder="Describe what happened during service and when." onChange={value=>update("serviceEvent",value)}/>
        <WorkspaceField number="2" title="Current symptoms" source="Entered by you" value={draft.currentSymptoms} placeholder="Describe what you experience now." onChange={value=>update("currentSymptoms",value)}/>
        ...
        <WorkspaceField number="4" title="Relationship to service" source="Your account, supported by reviewed sources" value={draft.relationship} placeholder="Explain when symptoms began or why you believe the details may be related." onChange={value=>update("relationship",value)}/>
        <WorkspaceField number="5" title="Daily impact" source="Entered by you" value={draft.dailyImpact} placeholder="Describe effects on work, movement, sleep, or daily activities." onChange={value=>update("dailyImpact",value)}/>
...
function PackageReview({claims,checks,approved,setApproved,unresolved,invalidateApproval,setDownloadOpen,onNavigate,onInspectSource,buddyDecision,setBuddyDecision}:{claims:ClaimWorkspace[];checks:PackageReadinessItem[];approved:boolean;setApproved:(value:boolean)=>void;unresolved:PackageReadinessItem[];invalidateApproval:(message:string)=>void;setDownloadOpen:(value:boolean)=>void;onNavigate:(screen:PrototypeScreen)=>void;onInspectSource:(sourceId:string)=>void;buddyDecision:BuddyDecision;setBuddyDecision:(value:BuddyDecision)=>void}){
  ...
  const citationCheck=checks.find(item=>item.id==="check-citation");
  ...
  return <div className="rw-page">
    ...
    {tab==="readiness"?<div className="rw-review-grid">
      ...
      <aside className="rw-panel rw-package-files"><span className="rw-kicker">Package contents</span><h2>Files assembled for review</h2><FileRow name="Package index.pdf" meta="Package overview and file checklist" ready={statementReady&&citationReady}/>{claims.map(item=><FileRow key={item.id} name={`${item.title} review.pdf`} meta="Statement, source trace, and open questions" ready={statementReady&&citationReady}/>)}<FileRow name="Supporting records/" meta="Linked documents, without duplicate copies" ready={citationReady}/><p><Info size={14}/> Neutral file status becomes ready only after the related review is complete.</p></aside>
