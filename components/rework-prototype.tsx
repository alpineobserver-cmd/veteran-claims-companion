  ...
  return <div className="rw-shell">
    ...
    <div className="rw-stage" inert={briefingOpen?true:undefined}>
      ...
      <main id="rework-main" tabIndex={-1}>
        ...
        {screen==="dashboard"&&<Dashboard services={services} events={events} documents={documents} leads={leads} claims={claims} packageKind={packageKind} onNavigate={navigate} onOpenClaim={openClaim}/>}
...
function Dashboard({services,events,documents,leads,claims,packageKind,onNavigate,onOpenClaim}:{services:ServicePeriod[];events:HealthEvent[];documents:DocumentRecord[];leads:ClaimLead[];claims:ClaimWorkspace[];packageKind:ClaimPath;onNavigate:(screen:PrototypeScreen)=>void;onOpenClaim:(claimId:string)=>void}){
  const accepted=leads.filter(item=>item.status==="accepted").length;
  const foundationReady=services.length>0&&events.length>0;
  const nextClaim=claims.find(claim=>!claim.draft.statementReviewed);
  const next=!foundationReady?{title:"Complete your foundation",copy:"Add at least one service period and one health event so claims can begin with useful context.",screen:"intake" as PrototypeScreen,label:"Open Service & Health"}:nextClaim?{title:`Continue ${nextClaim.title}`,copy:"Complete this claim's statement and verify its linked sources.",screen:"workspace" as PrototypeScreen,label:"Open Claim",claimId:nextClaim.id}:leads.some(item=>item.status==="open")?{title:"Review a claim lead",copy:"A topic appeared from information you added. Inspect its source before deciding whether to add a claim.",screen:"leads" as PrototypeScreen,label:"Review Leads"}:documents.length===0?{title:"Add records when you are ready",copy:"Your foundation is saved. Records can help Debrief identify facts for your review.",screen:"documents" as PrototypeScreen,label:"Open Documents"}:{title:"Add a claim topic",copy:"Add a condition or symptom yourself. You do not need to wait for a Debrief-generated lead.",screen:"leads" as PrototypeScreen,label:"Add a Claim"};
  return <div className="rw-page">
    ...
    <section className="rw-next"><span><ListChecks size={21}/></span><div><small>YOUR NEXT ACTION</small><h2>{next.title}</h2><p>{next.copy}</p></div><button className="rw-primary" type="button" onClick={()=>next.screen==="workspace"&&next.claimId?onOpenClaim(next.claimId):onNavigate(next.screen)}>{next.label} <ArrowRight size={15}/></button></section>
...
function PackageReview({claims,sources,approved,setApproved,invalidateApproval,setDownloadOpen,onNavigate,onOpenClaim,onInspectSource,buddyDecision,setBuddyDecision}:{claims:ClaimWorkspace[];sources:SourceReference[];approved:boolean;setApproved:(value:boolean)=>void;invalidateApproval:(message:string)=>void;setDownloadOpen:(value:boolean)=>void;onNavigate:(screen:PrototypeScreen)=>void;onOpenClaim:(claimId:string)=>void;onInspectSource:(sourceId:string)=>void;buddyDecision:BuddyDecision;setBuddyDecision:(value:BuddyDecision)=>void}){
  ...
  const readiness=claims.map(claim=>{
    ...
    return {claim,pathReady:claim.path!=="unsure",statementReady:claim.draft.statementReviewed,sourcesReady:unverified.length===0,unverified};
  });
...
function ReadinessLine({ready,label}:{ready:boolean;label:string}){return <div className={ready?"rw-readiness-line ready":"rw-readiness-line open"}><span>{ready?<Check size={13}/>:<Clock3 size={13}/>}</span><small>{label}</small></div>}
