  ...
  return <div className="rw-shell">
    ...
    <div className="rw-stage" inert={briefingOpen?true:undefined}>
      <header className="rw-topbar">
        ...
        <div><span>{onboardingComplete?"Your case":"New case setup"}</span><strong>{onboardingComplete?"Initial disability claim":"Build your case foundation"}</strong></div>
        <div className="rw-top-actions"><span className="rw-save"><Check size={13}/> Changes saved</span><button className="rw-guide" type="button" onClick={()=>setNotice(user.localTestProfile?"This local test profile saves only in this browser.":"This mock-up represents account-backed saving. Production integration will store case changes under your signed-in account.")}><Info size={17}/> <span>Save details</span></button><button className="rw-guide" type="button" onClick={()=>setBriefingOpen(true)}><CircleHelp size={17}/> <span>How Debrief works</span></button><Link className="rw-sign-in" href="/account"><UserRound size={15}/> {user.name||"Profile"}</Link></div>
      </header>
  ...
  return <div className="rw-modal-wrap rw-briefing-wrap" role="presentation">
    <section ref={dialogRef} className="rw-modal rw-briefing" role="dialog" aria-modal="true" aria-labelledby="mission-briefing-title" aria-describedby="mission-briefing-intro">
      ...
      <div className="rw-briefing-end">
        <p>{firstRun?"Next, add the service and health details you remember. You can return to this explanation from Help at any time.":"This explanation remains available here without occupying a permanent case step."}</p>
        <footer><button className="rw-primary" type="button" onClick={()=>onClose(true,firstRun)}>{firstRun?"Start intake":"Return to case"} <ArrowRight size={15}/></button></footer>
      </div>
    </section>
  </div>;
}
