...
export function DocumentIntake({initialWorkspaces,initialDocuments}:{initialWorkspaces:Workspace[];initialDocuments:DocumentRecord[]}){
  ...
  return <div className="intake-wrap">
    ...
    <div className="test-only-warning"><AlertTriangle size={20}/><div><strong>Fictional or synthetic files only</strong><p>Do not upload real medical records, names, Social Security numbers, VA file numbers, or information about another person. Malware scanning and production health-data controls are not yet complete.</p></div></div>
    ...
    <div className="intake-grid">
      <aside className="workspace-panel">
        ...
        <form className="new-workspace" onSubmit={createWorkspace}><label className="intake-field"><span>New workspace name</span><input value={workspaceName} onChange={event=>setWorkspaceName(event.target.value)} placeholder="For example, Synthetic review package" maxLength={160}/></label><button className="button secondary" disabled={!workspaceName.trim()||Boolean(working)}>{working==="workspace"?<LoaderCircle className="spin" size={15}/>:<FolderPlus size={15}/>}Create workspace</button></form>
      ...
      <main className="upload-panel">
        <div className="upload-heading"><div><span className="kicker">Synthetic upload</span><h2>Add a test document</h2></div><span>Maximum 4 MB</span></div>
        ...
        <label className="synthetic-confirm"><input type="checkbox" checked={confirmed} onChange={event=>setConfirmed(event.target.checked)}/><span><strong>I confirm this file is entirely fictional or synthetic.</strong><small>It contains no real person’s health information, identifiers, or records.</small></span></label>
