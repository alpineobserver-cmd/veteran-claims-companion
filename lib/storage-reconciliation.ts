...
export type ReconciliationInput={userId:string;operation:ReconciliationOperation;scope:string;entityId?:string;storageKey?:string;storageProvider?:string;storageZone?:DocumentStorageZone;reason:unknown};
...
export function reconciliationFingerprint(input:Omit<ReconciliationInput,"reason">,secret?:string){
  return rateLimitPrincipalHash(`storage-reconciliation:${input.operation}:${input.scope}:${input.entityId||"none"}:${input.storageProvider||"none"}:${input.storageZone||"primary"}:${input.storageKey||"none"}`,secret);
}
export async function recordStorageReconciliation(input:ReconciliationInput){
  ...
  try{
    await prisma.storageReconciliationTask.upsert({
      where:{fingerprint},
      create:{fingerprint,principalHash:principalHash(input.userId),operation:input.operation,scope:input.scope,entityId:input.entityId,storageKey:input.storageKey,storageProvider:input.storageProvider,storageZone:input.storageZone,lastErrorCode},
...
export async function retryUploadRollbackTasks(userId:string,storage:StorageProvider){
  let tasks;
  try{
    tasks=await prisma.storageReconciliationTask.findMany({where:{principalHash:principalHash(userId),operation:"DELETE_OBJECT",scope:"upload-rollback",status:"PENDING",storageKey:{not:null},AND:[{OR:[{storageProvider:storage.name},{storageProvider:null}]},{OR:[{storageZone:storage.zone||"primary"},{storageZone:null}]}]},orderBy:{createdAt:"asc"},take:10,select:{id:true,storageKey:true}});
  }catch(reason){
