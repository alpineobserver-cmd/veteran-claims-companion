import { z } from "zod";
import { buddyStatementProvenanceSchema } from "./buddy-statement";
import { generationAuditEntrySchema } from "./generation-audit";
...
export const claimDraftSchema = z.object({
  ...
  generationAudit: z.array(generationAuditEntrySchema).max(50).optional(),
  ...
  buddyStatements: z.array(z.object({id:z.string().max(100),witnessName:z.string().max(120),relationship:z.string().max(300),knownSince:z.string().max(300),observations:z.string().max(6000),specificExample:z.string().max(6000),changes:z.string().max(6000),statement:z.string().max(50_000),provenance:buddyStatementProvenanceSchema.optional(),createdAt:z.string().datetime(),updatedAt:z.string().datetime()}).strict()).max(10).optional()
}).strict();
