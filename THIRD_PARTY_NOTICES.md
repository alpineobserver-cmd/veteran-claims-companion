...
Reviewed against `package-lock.json` on August 21, 2026. Run `npm run licenses:check` to reproduce the production-package license classification. The package lock and the license files shipped with each installed package remain the exact dependency record.
...
| Vercel AI SDK and Google Vertex provider | Server-side structured personal-statement generation | Apache-2.0 | https://ai-sdk.dev and https://cloud.google.com/vertex-ai |
...
- **json-schema package** — offered under either AFL-2.1 or **BSD-3-Clause**. Debrief elects the BSD-3-Clause option and preserves the upstream copyright, conditions, and disclaimer distributed with the package.
...
The lockfile currently contains only MIT, ISC, Apache-2.0, BSD-2-Clause, BSD-3-Clause, 0BSD, CC-BY-4.0, LGPL-3.0-or-later, or explicitly reviewed alternatives and combinations for production dependencies. `scripts/check-third-party-licenses.mjs` fails when a new or unknown license expression enters the production graph so that this notice can be updated before release.
