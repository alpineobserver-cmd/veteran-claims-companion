...
Production uses `https://debriefclaims.com` as its sole canonical Alpha origin. Set `AUTH_URL` to that origin and `AUTH_CANONICAL_HOST` to `debriefclaims.com`; `www` and legacy Vercel addresses redirect to the apex before the application starts OAuth. Share only the canonical URL with testers.
...
The last step of the claim builder can turn a veteran's questionnaire answers into an editable personal-statement draft. The approved Alpha integration uses Gemini through Vertex AI with keyless Google workload identity, the fictional-data boundary, and the explicit cost controls documented in `.env.example`, `docs/durable-rate-limits.md`, and `docs/vertex-ai-alpha.md`.

Statement requests use a server-only provider adapter and schema-validated output. Per-user/global request and token limits plus an atomic daily spend reservation are checked before each provider request. The optional display name is added by the application and is not included in source material sent to the model. If Vertex is disabled or incomplete, the same interface creates a clearly labeled guided narrative directly from the answers using fixed, non-AI rules.
...
Run `npm run test:browser` to start an isolated local server and repeat the complete signed-out fictional claim, interruption recovery, PDF download, and large-workspace geometry checks in Chromium. Run `npm run test:browser:update` only when an intentional visual change requires reviewed macOS screenshot baselines. The browser harness disables uploads and external AI, creates no authentication bypass, and does not substitute for the disposable-account Google OAuth or physical-device checks documented in [the browser verification record](docs/browser-resilience-verification-2026-07-24.md).
...
Required Production authentication values are `AUTH_SECRET`, `AUTH_GOOGLE_ID`, `AUTH_GOOGLE_SECRET`, `AUTH_URL=https://debriefclaims.com`, and `AUTH_CANONICAL_HOST=debriefclaims.com`. Vercel masks secret values, so permanence is an operating control: confirm there is one Production `AUTH_SECRET`, do not replace it during normal releases, and record any intentional emergency rotation as an incident or planned maintenance event. A secret rotation signs users out and invalidates OAuth attempts already in progress.
