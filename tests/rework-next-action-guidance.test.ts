...
import {getPackageNextAction,getPackageReadiness,type ClaimLead,type ClaimWorkspace,type DocumentRecord,type HealthEvent,type ServicePeriod,type SourceReference} from "../lib/rework-prototype";
...
const failedDocument:DocumentRecord={id:"document-1",name:"Fictional record",type:"Medical record",dateRange:"2022",status:"failed",caseIds:["case-1"],claimIds:[],insights:[]};
...
test("90 modeled users receive one specific next action for their package state",()=>{
  ...
  const personas=Array.from({length:90},(_,index)=>({id:`NEXT-${String(index+1).padStart(3,"0")}`,family:index%6,device:["mobile","tablet","desktop"][index%3],experience:["first claim","returning","uncertain"][index%3]}));
