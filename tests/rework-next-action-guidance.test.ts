...
import {getPackageNextAction,type ClaimLead,type ClaimWorkspace,type HealthEvent,type ServicePeriod,type SourceReference} from "../lib/rework-prototype";
...
test("90 modeled users receive one specific next action for their package state",()=>{
  const expected=["foundation","claim-review","lead-review","add-claim","package-review"] as const;
  const personas=Array.from({length:90},(_,index)=>({id:`NEXT-${String(index+1).padStart(3,"0")}`,family:index%5,device:["mobile","tablet","desktop"][index%3],experience:["first claim","returning","uncertain"][index%3]}));
  const results=personas.map(persona=>{
    ...
    if(persona.family===2)return {persona,action:getPackageNextAction({...base,leads:[lead]})};
    if(persona.family===3)return {persona,action:getPackageNextAction(base)};
