...
| TM-05 | Malicious document, malware, or parser exhaustion | Extension/MIME/signature agreement; PDF/JPEG/PNG structural and decoded-size limits; active PDF and polyglot rejection; quotas and request limits; quarantine-only upload state, immutable GCS generation binding, private ClamAV scanner callback, and clean-only delivery code | Cloud Run/Eventarc buckets, definition freshness monitoring, provider agreement, and Staging acceptance evidence remain SEC-006 gates |
