...
- `/status` provides a browser-run snapshot of the public website, Google provider configuration, and anonymous session service without credentials or response-body logging. It explicitly does not claim authenticated save/export coverage or historical uptime.
- Repeated workflow failures open or update one privacy-safe GitHub issue containing only an observation time and workflow link; a later successful run closes the issue. The workflow never copies response bodies, account details, or claim information into the alert.
