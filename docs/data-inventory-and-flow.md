...
| `Claim.draftData` | Restricted questionnaire, timeline, statement, evidence-link, linked-record citation, privacy-minimized generation history, buddy-statement source trace, and package state | Exported; deleted with workspace/account. Generation history contains model/mode, policy version, field-name or timeline-position references, timestamps, result, and disposition but does not duplicate answers, statement text, document content, or prompts. |
...
| --- | --- | --- |
| `localStorage` key `vcc-claim-draft` | Signed-out `StoredDraft`: questionnaire answers, statement, provenance, timeline, evidence map, confirmations, document-link identifiers and user-entered page or section citations, revision history, privacy-minimized generation history, package state, and buddy statements with paragraph source traces | Same-origin JavaScript can read it; removed when transferred/cleared or by account deletion's `Clear-Site-Data`. It must remain fictional-only and is not acceptable for real health data without a new design. |
