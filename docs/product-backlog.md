...
- [ ] **OPS-014 · P2 · [Partially completed]** — Review the 13 foreign-key index recommendations reported by the Supabase performance advisor, add only query-supported indexes, and record before/after query plans. Completed independently before deployment July 22: mapped every recommendation to a real query/join/referential-cleanup path, captured privacy-safe Staging before plans, and added a tested Prisma migration with 13 ordered indexes. Closure requires the Staging deployment, advisor recheck, and recorded after plans.
