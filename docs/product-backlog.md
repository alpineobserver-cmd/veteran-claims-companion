...
- [ ] **SEC-002 · P0 · [Partially completed / Blocked]** — Develop authenticated, user-isolated storage approved for the intended information type. Authenticated owner-scoped database and private-object controls are implemented for fictional Alpha files; approval for real medical information remains blocked on the combined vendor, legal, threat-model, and independent-security review.
...
- [ ] **SEC-010 · P0 · [Blocked]** — Export minimal security events to protected monitoring without recording document contents, medical details, tokens, or identifiers that are not operationally necessary. Privacy-minimized application events exist, but the protected monitoring destination, access policy, retention, and alert ownership must be selected.
...
- [ ] **SEC-015 · P1 · [Partially completed]** — Add automated two-user authorization, upload/parser, account-deletion, API-abuse, and cross-origin tests. Expanded July 21: the release gate now directly exercises foreign-origin rejection, two fictional security principals, authentication/owner-scope contracts on every private API route, same-origin and durable-limit contracts on every authenticated mutation route, cross-user download-ticket rejection, adversarial upload/parser validation, verified account object deletion, and durable abuse limits. Closure requires a database-backed two-session route integration harness that proves user A cannot read, mutate, export, or delete user B's records through real handlers.
...
- [x] **OPS-006 · P1** — Add automated build, type, unit, integration, migration, dependency, security, and release-gate checks. Completed July 21: the protected `verify` gate now aggregates parallel TypeScript, production-build, deterministic unit/evaluation, storage/data-lifecycle integration, clean-PostgreSQL migration, dependency-tree/audit, and authentication/authorization/security jobs. High or critical dependency advisories block the release, and the existing local `test:release` command preserves the same complete regression coverage.
